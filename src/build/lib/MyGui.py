...
__title_format__="Helvetica 10 bold"
__column_format__="Helvetica 8 bold"
__item_format__="Helvetica 8"
...
class MyGui:
    def __init__(self):
        ...
        Label(self.menuBarFrame, width=10).grid(row=1, column=2)
        self.armyButton = Button(self.menuBarFrame, text="Quit", command=self.quit).grid(row=1, column=3)
        self.armyFrame = Frame(self.__mainWindow)
        self.armyFrame.grid(row=2, column=0, sticky='w')
        self.squadsFrame = Frame(self.__mainWindow)
        self.squadsFrame.grid(row=3, column=0, sticky = 'w')
        self.head = Label(self.squadsFrame, text ="                     Squad Name                             Type           Cost      Size")
        self.hum = Label(self.squadsFrame, text ="Humanoid Squads",font=__title_format__)
        self.veh = Label(self.squadsFrame, text ="Vehicle Squads",font=__title_format__)
        self.sqBars = []
        mainloop()
    ...
    def submit(self):
        ...
        if self._army.race != label:
            self._army = army.Army(label)
            self.updateArmy()
        for bar in self.sqBars:
            bar.frame.grid_remove()
        self.sqBars = []
        self.hum.grid_remove()
        self.head.grid_remove()
        self.veh.grid_remove()
        self.squadsFrame.grid_remove()
        self.squadsFrame.grid(row=3, column=0, sticky='w')
        self.hum = Label(self.squadsFrame, text ="Humanoid Squads",font=__title_format__)
        self.hum.grid(row=0,column=0,sticky='w')
        self.head = Label(self.squadsFrame, text ="                     Squad Name                             Type           Cost      Size")
        self.head.grid(row=1,column=0,sticky='w')
        for key in sorted(squad.Squads):
            if key != "" and squad.Squads[key].army_name == label:
                sq = SquadDisplayBar(self.squadsFrame, self, squad.Squads[key])
                self.sqBars.append(sq)
                sq.frame.grid()
        
        self.veh = Label(self.squadsFrame, text ="Vehicle Squads",font=__title_format__)
        self.veh.grid(column=0,sticky='w')
        for key in sorted(vehicle.Vehicles):
            if key != "" and vehicle.Vehicles[key].army_name == label:
                sq = SquadDisplayBar(self.squadsFrame, self, vehicle.Vehicles[key])
                self.sqBars.append(sq)
                sq.frame.grid()
    ...
    def removeCommand(self, remSquad):
        if isinstance(remSquad, vehicle.Vehicle):
            if(vehicle.Vehicles[remSquad.squad_name].squad_type) == "HQ":
                container = self._army.hq
            if(vehicle.Vehicles[remSquad.squad_name].squad_type) == "Troop":
                container = self._army.troop
            if(vehicle.Vehicles[remSquad.squad_name].squad_type) == "Fast Attack":
                container = self._army.fast
            if(vehicle.Vehicles[remSquad.squad_name].squad_type) == "Elite":
                container = self._army.elite
            if(vehicle.Vehicles[remSquad.squad_name].squad_type) == "Heavy Support":
                container = self._army.heavy
        else:
            if(squad.Squads[remSquad.squad_name].squad_type) == "HQ":
                container = self._army.hq
            if(squad.Squads[remSquad.squad_name].squad_type) == "Troop":
                container = self._army.troop
            if(squad.Squads[remSquad.squad_name].squad_type) == "Fast Attack":
                container = self._army.fast
            if(squad.Squads[remSquad.squad_name].squad_type) == "Elite":
                container = self._army.elite
            if(squad.Squads[remSquad.squad_name].squad_type) == "Heavy Support":
                container = self._army.heavy
...
class SquadDisplayBar:
    def __init__(self, parent, guiClass, squad):
        ...
        self.size = squad.current_size
        self.frame = Frame(parent, borderwidth=2)
        name = Label(self.frame, width=30, height = 2, justify=LEFT, text = self.name)
        sq_type = Label(self.frame, width=12, height = 2, text = self.type)
        cost = Label(self.frame, width=6, height = 2, text = self.cost)
        size = Label(self.frame, width=4, height = 2, text = self.size)
...
class SimulateWindow:
    """
    "   Class which will display the results of combat simulation, and how many simulations to run
    """
    ...
    def runSim(self):
        """
        "   For the number of times given in the spinbox, perform ranged combat simulation
        "   Results are displayed as a percentage of times that the outcome was reached
        """
        self.simKillResults = {}
        self.simHitResults = {}
...
class SquadWindow:
    """
    " Class which is used to display and alter values for a squad through the GUI, including addition of units to squad and squad to army
    """
    def __init__(self, parent, squadName):
        ...
        if isinstance(self.squad, squad.Squad):
            ...
            self.wepSpin = Spinbox(self.__mainWindow, width=50,values=weps)
        ...
        self.fillButton.grid(row=0, column=5)
